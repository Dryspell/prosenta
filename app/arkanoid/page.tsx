...
function Ball() {
  ...
  const onCollisionEnter = () => (
    // @ts-expect-error
    ref.current.setTranslation({ x: 0, y: 0, z: 0 }), ref.current.setLinvel({ x: 0, y: 10, z: 0 })
  )
  return (
    <>
      {/* @ts-expect-error */}
...
function Paddle({ euler = new THREE.Euler(), quaternion = new THREE.Quaternion() }) {
  ...
  useFrame(({ pointer, viewport }) => {
    ...
    ref.current && ref.current.setRotation(quaternion.setFromEuler(euler.set(0, 0, (pointer.x * Math.PI) / 10)))
  })
  return (
