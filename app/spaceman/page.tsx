...
import { Box } from 'src/components/canvas/Examples'
import { Ninja } from 'src/components/models/Ninja'
extend({ OrbitControls, Environment, Float, PivotControls, QuadraticBezierLine, Backdrop, ContactShadows })
...
function Cable({ start, end, v1 = new THREE.Vector3(), v2 = new THREE.Vector3() }) {
  const ref = useRef<typeof QuadraticBezierLine>()
  //@ts-expect-error
  ...
  return (
    <Canvas shadows camera={{ position: [0, 1.5, 3] }}>
      ...
      <Float
        ...
        speed={1.5}
      >
        <Ninja scale={0.2} hovered={false}>
          <object3D position={[-0.6, 2, 0]} ref={spaceman} />
        </Ninja>
      </Float>
      <Cable start={ship} end={spaceman} />
      <Backdrop castShadow floor={2} position={[0, -0.5, -3]} receiveShadow scale={[50, 10, 4]}>
